...
import (
	...
	"MIS-BC/security/minsecurity/crypto/sm3"
	"testing"
)
...
func TestSignAndVer(t *testing.T) {
	msg := []byte("sm2_general message0000")
	priv, err := generateKey(rand.Reader)
	if err != nil {
...
func TestSignAndVerWithAsn1(t *testing.T) {
	msg := []byte("sm2_general message")
	priv, pubk := GenKeyPair()

	sig, err := priv.Sign(rand.Reader, msg, nil)
	if err != nil {
		t.Fatalf("sm2_general sign failed:%s", err)
	}
	var h sec.Hash
	if ok, _ := pubk.Verify(msg, sig, h); !ok {
...
func BenchmarkVerifyWithASN1(b *testing.B) {
	...
	var h sec.Hash
	for i := 0; i < b.N; i++ {
		pubk.Verify(msg, sig, h)
	}
}
func TestEncAndDec(t *testing.T) {
	msg := []byte("sm2_general encryption standard")
	...
	if !bytes.Equal(msg, plain) {
		t.Fatal("sm2_general encryption is invalid")
	}
}
